...
Scrapes Q Guide data and exports to CSV format.
"""
...
def main():
    """Main function to scrape Q Reports HTML files."""
    # Look for HTML files in the data directory
    html_files = glob('data/old_html/QReports_*.htm*')
    
    if not html_files:
        print("ERROR: No Q Reports HTML files found in data/old_html/")
        print("Please place Q Reports HTML files in scraper/data/old_html/")
        return
    ...
    output_file = 'output/courses_by_fas_id.json'
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(merged_courses, f, indent=2, ensure_ascii=False)
    
    # Print statistics
    print(f"\n{'='*60}")
    print(f"Total unique FAS IDs: {len(merged_courses)}")
